...
const data = ["About Us" , "Services" , "Portfolio" , "Careers" , "Contact Us" , "Business Enquiries"]
  ...
  <div className={`w-full flex items-center justify-between bg-black   xl:flex-col  fixed xl:px-[20px] px-[40px] py-[20px] xl:py-[20px] z-30 `}  >
...
<div>hellp</div>
 ...
 <div className={`${isContentVisible ? 'xl:flex' : 'xl:hidden'} flex items-center xl:items-start xl:px-[px]  flex-row xl:flex-col gap-10 xl:gap-0 py- xl:bg-transparent xl:bg-black xl:w-screen xl:h-screen xl:absolute md:top-[76px]  xl:top-[90px] xl:pb-[200px]  overflow-auto`}>
    {data.map((data, index) => (
        <ul key={index} className='flex flex-col gap-5 font-bold  xl:text-l uppercase xl:relative  ' style={{color:Color}}>
            {data === "Business Enquiries" ? (
                <li className=' xl:py-2 xl:px-10 text-white'        onClick={() => setstartproject(!startproject)}><Button data={data} width={'230px'} /></li>) : ( <NavLink
...
{/* <NavLink to={`/${data}`}> */}
    ...
    <div className={`tracking-tight  cursor-pointer ${border} text-white`}>{data}</div>
      {/* <p className='min-xl:hidden'>{'>'}</p> */}
      <p className='min-xl:hidden'><Megamenubtn/></p>
    </div>
    }

    <div class="absolute xl:relative hidden bg-black min-xl:shadow-lg p-4 top-[55px] xl:top-[0px]  mt-2 group-hover:block  border-[0.1px] border-gray-300">
  ...
  services_mega_menu.map((data, index) => (
      <ul className='flex ' key={index}>
       <Link to={`/${data}`}>
        <li className='cool-link  font-[500]  text-white '>{data}</li>
        </Link>
  ...
  (
    <li className={`tracking-tight  cursor-pointer xl:border-t-2  xl:py-2 xl:px-10 xl:w-screen   ${border}  text-white `}>{data}</li>
  )
